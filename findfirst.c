...
int _findnext(intptr_t fhandle, struct _finddata_t* fileinfo) {
    ...
    while ((result = readdir(handle->dstream)) != NULL) {
        if (!handle->dironly && !match_spec(handle->spec, result->d_name)) {
            continue;
        }

        if (fstatat(dirfd(handle->dstream), result->d_name, &st, 0) == -1) {
            return -1;
        }
        ...
        fill_finddata(&st, result->d_name, fileinfo);
